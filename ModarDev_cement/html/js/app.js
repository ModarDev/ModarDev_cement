...
var HitDistance = 50; // hit distance in px
var killTimeout;
var active = false;
...
(() => {
    ...
    CraftGame.Close = function() {
        ...
        $.post("http://Fewthz_cement/CloseGame", JSON.stringify({}));
    };

    CraftGame.Failed = function() {
        if (active) $.post("http://Fewthz_cement/Revivefalse",'{}');
        active = false;
        CraftGame.Close();
    }

    CraftGame.Success = function() {
        if (active) $.post("http://Fewthz_cement/Reviveok",'{}');
        active = false;
        CraftGame.Close();
    }
